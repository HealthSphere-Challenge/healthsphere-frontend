...
| Consumer contract | Backend fixtures/schema revision via HS-002 | Response validation, units, optional/null semantics, errors and incompatible versions; only backend requests |
...
CI minimum: locked dependency install, lint, strict typecheck, unit/component tests and production build. Add Playwright/axe release workflows when vertical functionality exists. Package manager, scripts and CI versions are unresolved until HS-003; do not document commands as runnable before they exist.
