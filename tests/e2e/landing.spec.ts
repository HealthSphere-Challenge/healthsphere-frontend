...
test('How it works reaches the real section', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/#how-it-works$/); await expect(page.getByRole('heading', { name: 'How HealthSphere works' })).toBeVisible()
})
...
for (const width of [1440, 1024, 768, 390]) test(`landing reflows at ${width}px`, async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Important context stays visible.' })).toBeVisible()
  expect(await page.evaluate<boolean>('[...document.querySelectorAll(\'img[loading="lazy"]\')].every((image) => image.naturalWidth > 0)')).toBe(true)
