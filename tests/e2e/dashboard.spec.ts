...
test('dashboard renders a truthful empty state without risk claims', async ({ page }) => {
  ...
  await expect(page.getByText(/risk|healthy|diagnos/i)).toHaveCount(0)
})
...
for (const width of [1440, 1024, 768, 390]) test(`dashboard reflows at ${width}px`, async ({ page }) => { await authenticated(page, true); await page.setViewportSize({ width, height: 900 }); await page.goto('/app'); await expect(page.getByRole('heading', { name: /Good to see you/ })).toBeVisible() })
