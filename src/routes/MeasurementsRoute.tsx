...
function MeasurementForm({ initialMetric, onClose }: { initialMetric: FormValues['metric']; onClose: () => void }) {
  ...
  const form = useForm<FormValues>({ resolver: zodResolver(formSchema), defaultValues: { metric: initialMetric, value: 0, measured_at: now, note: '' } }); const metric = useWatch({ control: form.control, name: 'metric' })
