...
function Capabilities() {
  return <section className="landing-capabilities" id="features" aria-labelledby="features-title"><div className="landing-section landing-capabilities__inner"><header><p className="eyebrow">Product capabilities</p><h2 id="features-title">Track, review and understand the information you record.</h2><p>Each part of HealthSphere uses the same saved context, without turning missing information into a health conclusion.</p></header><div className="landing-capability-layout">{capabilities.map(([title, text]) => <article key={title}><h3>{title}</h3><p>{text}</p></article>)}<figure><img src={olderCouple} width="1200" height="765" loading="lazy" decoding="async" alt="Older adults using a laptop together." /><figcaption>Designed for the information you choose to record and revisit.</figcaption></figure></div><Link className="landing-text-link" to="/register">Create your HealthSphere account <span aria-hidden="true">→</span></Link></div></section>
}

function Safety() {
  return <section className="landing-trust" id="safety" aria-labelledby="safety-title"><div className="landing-section landing-trust__inner"><div className="landing-trust__content"><header><p className="eyebrow">Safety and limitations</p><h2 id="safety-title">Important context stays visible.</h2><p>HealthSphere provides health information, not diagnosis. It does not prescribe treatment or replace a qualified healthcare professional.</p></header><dl><div><dt>Experimental assessment</dt><dd>Trained on synthetic Synthea records, uncalibrated and not clinically validated.</dd></div><div><dt>Grounded Assistant</dt><dd>Uses available medical-information sources, communicates uncertainty and escalates urgent signals.</dd></div><div><dt>Privacy-conscious flow</dt><dd>Your browser talks to the HealthSphere backend; it does not call model or provider services directly.</dd></div><div><dt>Accessible by design</dt><dd>Clear labels, keyboard access, readable contrast and responsive layouts are part of the product baseline.</dd></div></dl></div><figure><img src={olderWomanCare} width="1200" height="800" loading="lazy" decoding="async" alt="Older adult talking with a healthcare professional." /></figure></div></section>
}
